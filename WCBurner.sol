...
contract WCBurner is AccessControl, ERC721Holder {
    ...
    function setWC(address WC) public onlyRole(DEV_ROLE) {
        _WC = WC;
    }
    ...
    function doBurn(uint256 amount) public onlyRole(DEV_ROLE) {
        IERC721 nftContract = IERC721(_WC);
        for (uint256 i = 0; i < amount; i++) {
            nftContract.mintCredit();
            uint256[] memory tokens = nftContract.tokensOfOwner(address(this));
            nftContract.safeTransferFrom(address(this), address(1), tokens[tokens.length-1]);
        }
    }
}
