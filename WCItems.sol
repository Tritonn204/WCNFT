...
contract WCItems is ERC1155, AccessControl, ERC1155Burnable, ERC1155Supply {
    ...
    mapping(uint256 => uint256[]) public _itemMachines;
    mapping(uint256 => bool) public _isMachineOpen;
    
    mapping(uint256 => uint256) public _upgradePath;
    uint256 public _upgradeCost = 5;

    mapping(uint256 => string) public _uris;

    constructor() ERC1155("WC Items") {
    ...
    function updateDurability(uint256 itemId, uint256 durability) public onlyRole(DEV_ROLE) {
        _baseDurability[itemId] = durability;
    }

    function setUpgradeCost(uint256 newCost) public onlyRole(DEV_ROLE) {
        _upgradeCost = newCost;
    }

    function setUpgradePath(uint256 baseId, uint256 upgradedId) public onlyRole(DEV_ROLE) {
        _upgradePath[baseId] = upgradedId;
    }

    function setToken(address newToken) public onlyRole(DEV_ROLE) {
        _tokenAddress = newToken;
    }

    function setDuelContract(address duelContract) public onlyRole(DEV_ROLE) {
    ...
    function updateInventory(uint256 machineId, uint256[] calldata itemOptions) public onlyRole(DEV_ROLE) {
        _itemMachines[machineId] = itemOptions;
    }


    function enableMachine(uint256 machineId) public onlyRole(DEV_ROLE) {
        require(_isMachineOpen[machineId] == false, "machine is already enabled");
        _isMachineOpen[machineId] = true;
    }

    function disableMachine(uint256 machineId) public onlyRole(DEV_ROLE) {
        require(_isMachineOpen[machineId] == true, "machine is already disabled");
        _isMachineOpen[machineId] = false;
    }
    ...
    function setURI(uint256 tokenId, string calldata URI) public onlyRole(DEV_ROLE) {
        _uris[tokenId] = URI;
    }

    function randomNumber(uint256 salt) private view returns(uint256){
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp + salt)));
    }

    function purchaseItems(uint256 machineId, uint256 pack, bytes memory data) public {
        require(_isMachineOpen[machineId], "invalid request (machineId)");
        uint256[] memory tokenIds = new uint256[](_packs[pack]);
        uint256[] memory amounts = new uint256[](_packs[pack]);
    ...
    function upgradeEquipment(uint256 itemId) public {
        require(balanceOf(msg.sender, itemId) >= _upgradeCost, "insufficient balance");
        require(_upgradePath[itemId] != 0, "this item cannot be upgraded");

        burn(msg.sender, itemId, _upgradeCost);
        mint(msg.sender, _upgradePath[itemId], 1, "0x");
    }
